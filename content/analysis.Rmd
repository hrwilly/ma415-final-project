...
\
We then faceted by team to observe the relationship between payroll and winning percentage for each team. We performed linear regression for all teams and provided the slopes in a table and bargraph.\
\
...
```
```{r coefs, fig.width = 8, fig.height = 6, echo = FALSE}
...
slopes %>% 
  kbl(col.names = c("Team", "Slope", "Standard Error", "P Value"), align = "c") %>% 
  kable_paper(full_width = TRUE, "hover") %>% 
  kable_classic_2(html_font = "Georgia") %>% 
  kable_styling(position = "center") %>% 
  scroll_box(height = "200px", width = "100%")
