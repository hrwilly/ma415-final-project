...
```
\
To our surprise, for 11 out of the 15 teams, as payroll increases, the total number of hits in a season decreases. This suggests that even though teams are spending more money on more valuable players, their offensive performance in their total hits for a season does not increase.\
\
We then looked at the relationships between payroll and homeruns for each season. It is possible that teams are spending more money on players known for hitting home runs and therefore bring in more runs rather than players who just hit and get on base without bringing in a run. We first calculated the total home runs for each team each year by summing their home home runs and their away home runs. We then joined this dataset with the payroll dataset and plotted the total homeruns of each team against their payroll for each year between 2000 and 2016.\
\
...
\
As a result of our findings from the analysis of the positive relationship between payroll and winning percentage, we chose to focus on what pitchers Oakland lost from the 2013 season to the 2014 season. We first summed the number of home games and away games each starting pitcher started for the year of 2013. We then summed the number of times each starting pitcher was named the winning pitcher. A [winning pitcher](https://www.mlb.com/glossary/standard-stats/win) is defined as the pitcher who last pitched when his team takes and maintains the lead. We then divided each pitcher's winning pitcher sum and games started sum and called this the pitcher's winning proportion. We repeated this process for the 2014 season and filtered only those pitchers who pitched over 30 games to compare winning proportions. The results are provided in the following tables.\
\
...
```
\
When comparing the winning proportions of the top Oakland pitchers for the 2013 and 2014 seasons, we see one pitcher stand out: [Bartolo Colón](https://www.mlb.com/player/bartolo-colon-112526). With a winning proportion of about 60%, Colón was traded to the New York Mets in 2014. Colón, along with two other pitchers who started in more than 30 games, did not return to Oakland for the 2014 season. In 2014, Oakland dropped to only having two pitchers start in over 30 games with winning proportions more than 10% less than Colón's. 
\
...
Similar to the Oakland Athletics, we calculated the winning proportion for those starting pitchers who started in at least 30 games for the 2013 and 2014 seasons of the Atlanta Braves.\
\
...
To conclude our analysis, we took all the variables that we previously observed (payroll, hits, homeruns, strikeouts, and earned runs against) and conducted a forward selection stepwise regression to see how well these variables can predict the response variable of winning percentage. \
\
...
```
\
Table 9 below contains the estimates for the best model with the predictor variables total strikeouts, total hits, total homeruns, and total earned runs. 
\
...
all variables act how we expect: home runs most important for a win as well as lower number of opposing teams' earned runs (makes sense because you are more likely to win if the other team scores less), number of hits positively correlated as we expect
number of strikeouts technically negative but significantly less important than other variables (?) something to do woth the fact that its the only probability thats greater than zero? idk what the probability means in the table but could maybe make the connection here

payroll not being good predictor:
accurate to our thesis that payroll doesnt correspond and also accurate to the cool bar graph of the slopes
doesn't matter how much money you have, but rather on how you choose to spend it

## Limitations

There are a few limitations in our analysis, specifically for the teams that have a negative relationship between payroll and winning percentage. We only analyzed two of the fifteen total teams that have that negative relationship, then generalized  our findings. It could be true that what we find for those two teams is not the same for the other thirteen teams not analyzed. We also only did analysis of the pitchers for the teams with the negative relationship, and did not do analysis of other defensive positions, or any offensive factors.