...
\
```{r win_perc, eval = FALSE, echo = FALSE}
...
```
\
Next, we looked at the Chicago White Sox.
\
```{r CHA, echo = FALSE}
...
kable((cha[, 1:5]),
        caption = "Chicago White Sox Highest and Lowest Payroll Years",
        col.names = c("Year", "Winning Percentage", "Payroll (millions)", "Payroll Difference from Previous Year", "Winning Percentage Difference from Previous Year"),
        align = "c",
        digits = 2) %>% 
kable_paper(full_width = TRUE, "hover") %>% 
...
```
\
For the Chicago White Sox, the largest payroll increase from one year to the next was from the year 2000 to 2001. From these two years there was a \$31 million increase in their payroll with their winning percentage decreasing by 7.41%. Similar to the Oakland Athletics, we looked at what pitchers the White Sox lost from 2000 to 2001 resulting in this decreased winning percentage despite a \$31 million increase in payroll.\

```{r CHA_pitchers, echo = FALSE}
#2000
home3 <- baseball %>% filter(h_name == "CHA", year == 2000) %>% 
  group_by(h_starting_pitcher_name) %>% 
  summarize(h_started = n(), 
            h_won = sum(h_starting_pitcher_name == winning_pitcher_name)) %>% 
  arrange(desc(h_started))

away3 <- baseball %>% filter(v_name == "CHA", year == 2000) %>% 
  group_by(v_starting_pitcher_name) %>% 
  summarize(v_started = n(), 
            v_won = sum(v_starting_pitcher_name == winning_pitcher_name)) %>% 
  arrange(desc(v_started))
...
#2001
home4 <- baseball %>% filter(h_name == "CHA", year == 2001) %>% 
  group_by(h_starting_pitcher_name) %>% 
  summarize(h_started = n(), 
            h_won = sum(h_starting_pitcher_name == winning_pitcher_name)) %>% 
  arrange(desc(h_started))

away4 <- baseball %>% filter(v_name == "CHA", year == 2001) %>% 
  group_by(v_starting_pitcher_name) %>% 
  summarize(v_started = n(), 
            v_won = sum(v_starting_pitcher_name == winning_pitcher_name)) %>% 
  arrange(desc(v_started))
...
pitchers00 <- home3 %>% left_join(away3, by = c("h_starting_pitcher_name" = "v_starting_pitcher_name")) %>% 
  replace(is.na(.),0) %>% 
  mutate(games_started = h_started + v_started, 
         games_won = h_won + v_won, 
         win_prop = games_won/games_started) %>% 
...
tib0 <- tibble(h_starting_pitcher_name = "Others",
       games_started = sum(pitchers00$games_started[3:13]),
       games_won = sum(pitchers00$games_won[3:13]),
       win_prop = games_won/games_started)

top_perc0 %>% add_row(tib0) %>% 
  ggplot(aes(x = reorder(h_starting_pitcher_name, win_prop), y = win_prop, fill = h_starting_pitcher_name)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Starting Pitcher Name", y = "Winning Pitcher Proportion") +
  theme(legend.position = "none")


pitchers01 <- home4 %>% left_join(away4, by = c("h_starting_pitcher_name" = "v_starting_pitcher_name")) %>% 
  mutate(games_started = h_started + v_started, 
         games_won = h_won + v_won, 
         win_prop = games_won/games_started) %>% 
  select(h_starting_pitcher_name, games_started, games_won, win_prop)

tib1 <- tibble(h_starting_pitcher_name = "Others",
       games_started = sum(pitchers01$games_started[2:11]),
       games_won = sum(pitchers01$games_won[2:11]),
       win_prop = games_won/games_started)

top_perc1 %>% add_row(tib1) %>% 
  ggplot(aes(x = reorder(h_starting_pitcher_name, win_prop), y = win_prop, fill = h_starting_pitcher_name)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Starting Pitcher Name", y = "Winning Pitcher Proportion") +
...
we only looked at loss of pitchers for negative relationship, we didnt look at any of the other positions.
