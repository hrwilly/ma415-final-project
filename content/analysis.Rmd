...
highest %>% 
  ggplot(aes(y = h_name, fill = h_name)) +
  geom_bar() +
  coord_polar(theta = "y") +
  theme(legend.position = "none",
        panel.background = element_rect(fill = "lightblue", color = "lightblue")) +
  labs(title = "Number of Years Each Team Had Highest Winning Percentage", x = "Number of Years", y = "Team") +
...
matches %>% 
  kbl(col.names = c("Year", "Highest Winning Percentage", "Most Hits", "Most Homeruns", "Most Runs"), align = "c") %>% 
  kable_paper(full_width = TRUE, "hover") %>% 
  kable_classic_2(html_font = "Georgia") %>% 
  kable_styling(position = "center") %>% 
  scroll_box(height = "200px", width = "100%")
...
ggplot(aes(payroll,win_perc)) + 
  ...
  labs(x = "Winning Percentage", y = "Payroll", 
    title = "Relationship Between Winning Percentage and Payroll for Each Team") +
...
\
From these faceted plots, it is not consistent among all teams that as payroll increases, winning percentage also increases. For exactly half of the MLB teams, as payroll increases, their winning percentage for that year decreases.\
\
...
payroll_hits %>%
  ...
  facet_wrap(vars(h_name), scales = "free", ncol = 5) +
  labs(x = "Total Hits", y = "Payroll", 
    title = "Relationship Between Total Hits and Payroll for 15 Positive Relationship Teams") +
  scale_fill_discrete(name = "Year") +
  theme(strip.text.x = element_text(size = 12, face = "bold")) +
  theme(strip.background = element_rect(color = "black", fill = "#F0F8FF", size = 1, linetype = "solid"))
```
...
payroll_hruns %>%
  ...
  geom_smooth(formula = y ~ x, method='lm', color = "red", lwd = 0.8, se = FALSE) +
  facet_wrap(vars(h_name), scales = "free", ncol = 5) +
  labs(x = "Total Homeruns", y = "Payroll", 
    title = "Relationship Between Total Homeruns and Payroll for 15 Positive Relationship Teams") +
  scale_fill_discrete(name = "Year") +
  theme(strip.text.x = element_text(size = 12, face = "bold")) +
  theme(strip.background = element_rect(color = "black", fill = "#F0F8FF", size = 1, linetype = "solid"))
```
...
payroll_so %>%
  ...
  geom_smooth(formula = y ~ x, method='lm', color = "red", lwd = 0.8, se = FALSE) +
  facet_wrap(vars(h_name), scales = "free", ncol = 5) +
  labs(x = "Total Strikeouts", y = "Payroll", 
    title = "Relationship Between Total Strikeouts and Payroll for 15 Positive Relationship Teams") +
  scale_fill_discrete(name = "Year") +
  theme(strip.text.x = element_text(size = 12, face = "bold")) +
  theme(strip.background = element_rect(color = "black", fill = "#F0F8FF", size = 1, linetype = "solid"))
```
...
payroll_er %>%
  ...
  geom_smooth(formula = y ~ x, method='lm', color = "red", lwd = 0.8, se = FALSE) +
  facet_wrap(vars(h_name), scales = "free", ncol = 5) +
  labs(x = "Total Earned Runs", y = "Payroll", 
    title = "Relationship Between Total Earned Runs and Payroll for 15 Positive Relationship Teams") +
  scale_fill_discrete(name = "Year") +
  theme(strip.text.x = element_text(size = 12, face = "bold")) +
  theme(strip.background = element_rect(color = "black", fill = "#F0F8FF", size = 1, linetype = "solid"))
```
