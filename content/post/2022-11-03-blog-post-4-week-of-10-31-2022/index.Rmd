...
(pitcher_win<-baseball %>% filter(h_name=="BOS",h_score>v_score) %>% 
                           group_by(h_starting_pitcher_id,year) %>% 
                           count(h_starting_pitcher_id) %>% 
                           rename(games_won=n))
...
```{r graph1cont,fig.height=10}

pitcher_prop %>% group_by(year) %>%
                ggplot(aes(h_starting_pitcher_id,prop,fill=h_starting_pitcher_id))+
...
pitcher_win %>% group_by(year) %>%
                ggplot(aes(h_starting_pitcher_id,games_won,fill=h_starting_pitcher_id))+
                geom_col(show.legend=FALSE)+
                coord_flip()+
                facet_wrap(vars(year),scales="free_y",ncol=3)+
