...
(pythag <- baseball %>% mutate(h_win_loss = h_win_loss(h_score, v_score)))

#for Boston Red Sox, grouped by season/year, actual winning percentage
h_wins <- pythag %>% filter(h_name == "BOS") %>% group_by(year) %>% summarize(home_games_won = sum(h_win_loss == 1))
a_wins <- pythag %>% filter(v_name == "BOS") %>% group_by(year) %>% summarize(away_games_won = sum(h_win_loss == 0))
wins <- h_wins %>% left_join(a_wins, by = "year") %>% mutate(games_won = home_games_won + away_games_won)

(win_percentage <- pythag %>% filter(h_name == "BOS" | v_name == "BOS") %>% 
  group_by(year) %>% 
