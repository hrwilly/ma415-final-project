...
This data set does not specify whether the home team wins or loses, but it does contain information on the home team's and the away team's number of runs. Since we are interested in seeing if a team wins or loses, we created a column in the dataset called `score_diff`, which takes the number of the visiting team's runs subtracted from the number of the home team's runs. A positive value represents a win, a negative value represents a loss, and a zero is a tie. The higher absolute values represent a bigger win/loss.
...
baseball %>% group_by(h_name) %>% 
  ...
  kbl(caption = "Average Runs Scored by Home Teams from 2000 to 2016", align = "c", col.names = c("Home Team Name", "Average Runs Scored")) %>% 
...
baseball %>% group_by(v_name) %>% 
  ...
  arrange(desc(average_score)) %>% 
  kbl(caption = "Average Runs Scored by Away Teams from 2000 to 2016", align = "c", col.names = c("Away Team Name", "Average Runs Scored")) %>% 
  kable_classic_2("striped", html_font = "Georgia") %>% 
  kable_styling(position = "center")

#Proportion of games won by home team
baseball %>% group_by(h_name) %>% summarize(win_prop = round(mean(score_diff>0), 4)) %>% 
  arrange(desc(win_prop)) %>% 
  kbl(caption = "Proportion of Games Won by Home Team from 2000 to 2016", align = "c", col.names = c("Home Team Name", "Proportion of Games Won")) %>% 
  kable_classic_2("striped", html_font = "Georgia") %>% 
  kable_styling(position = "center")
...
baseball %>% group_by(v_name) %>% 
  ...
  arrange(desc(win_prop)) %>% 
  kbl(caption = "Proportion of Games Won by Away Team from 2000 to 2016", align = "c", col.names = c("Away Team Name", "Proportion of Games Won")) %>% 
  kable_classic_2("striped", html_font = "Georgia") %>% 
  kable_styling(position = "center")
```
